...
struct LZMALenDecoder {
    ...
    mutating func decode(with rangeDecoder: inout LZMARangeDecoder, posState: Int) throws -> Int {
        ...
        if try rangeDecoder.decode(bitWithProb: &self.choice) == 0 {
            return try self.lowCoder[posState].decode(with: &rangeDecoder)
        }
        // If binary sequence starts with 1 0 then:
        if try rangeDecoder.decode(bitWithProb: &self.choice2) == 0 {
            return 8 + (try self.midCoder[posState].decode(with: &rangeDecoder))
        }
        // If binary sequence starts with 1 1 then:
        return 16 + (try self.highCoder.decode(with: &rangeDecoder))
    }
